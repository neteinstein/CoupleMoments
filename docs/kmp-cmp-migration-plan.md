# Plan: Migrate Couple Moments to Kotlin Multiplatform + Compose Multiplatform

**Status:** proposed, not yet implemented.
**Reference implementation:** [`neteinstein/loopgain`](https://github.com/neteinstein/loopgain) — same
author, same stack (KMP + CMP, Koin, Ktor, Coil), already shipping Android and building iOS from one
shared module. This plan reproduces its shape and calls out every place Couple Moments' current
feature set (Room-backed question deck, GitHub self-update, per-app language, intimacy gate,
"questions for parents" toggle) needs a different answer than LoopGain's simpler settings-only app.

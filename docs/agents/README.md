...
| Product Manager | [`product-manager.md`](./product-manager.md) | [`.claude/agents/product-manager.md`](../../.claude/agents/product-manager.md) |
...
Architect, Security Manager, and Product Manager are the three orchestrator roles: they're the ones expected to plan/review and then hand work off rather than doing all the typing themselves. Developer and QA are workers — they don't spawn other agents.
...
Product Manager ──┬──> Developer   (fix a usability/product finding)
                   └──> QA         (add coverage for a usability edge case)
```

Only `architect`, `security-manager`, and `product-manager` get the `Agent` tool in their frontmatter, so only they can spawn `developer`/`qa`/each other via the `Agent` tool, `@agent-<name>` mentions, or plain natural language ("use the qa subagent to..."). `developer` and `qa` omit `Agent` entirely — they're leaves.
